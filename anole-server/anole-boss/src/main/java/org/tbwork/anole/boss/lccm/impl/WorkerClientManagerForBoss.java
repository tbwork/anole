package org.tbwork.anole.boss.lccm.impl;
...
import org.tbwork.anole.boss.lccm.skeleton.WorkerClientSkeleton;
import org.tbwork.anole.boss.lccm.skeleton.WorkerClientSkeleton.CustomerClient;
import org.tbwork.anole.boss.lccm.util.ClientEntropyUtil; 
...
public class WorkerClientManagerForBoss extends LongConnectionClientManager {
	...
	public void unregisterClient(UnregisterRequest request) { 
		WorkerClientSkeleton wc =  (WorkerClientSkeleton) longConnectionClientMap.get(request.getClientId());
		if(wc != null){ 
	...
	public void updateStatus(int clientId, int subscriberClientCount, int weight){
		WorkerClientSkeleton wc = (WorkerClientSkeleton)  longConnectionClientMap.get(clientId);
		 if(wc != null){ 
	...
	public WorkerClientSkeleton selectBestWorkerForSubscriber(){
		return ClientEntropyUtil.selectBestWorker(ClientType.SUBSCRIBER, longConnectionClientMap);
	}
