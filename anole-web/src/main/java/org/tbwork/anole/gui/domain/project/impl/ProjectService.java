...
import org.anole.infrastructure.dao.extend.AnoleProductLineExtMapper;
import org.anole.infrastructure.dao.extend.AnoleProjectExtMapper;
...
public class ProjectService implements IProjectService{
	...
	@Autowired
	private AnoleProjectExtMapper anoleProjectExtMapper;
	
	@Autowired
	private AnoleProductLineMapper anoleProductLineMapper;

	@Autowired
	private AnoleProductLineExtMapper anoleProductLineExtMapper;
	...
	private void initialize(){
		...
		List<AnoleEnvironment> envs = anoleEnvironmentMapper.selectByExample(new AnoleEnvironmentExample());
	...
	public CreateProjectResult createProject(CreateProjectDemand createProjectDemand) {
		...
		try{
			...
			AnoleProductLine apl = anoleProductLineExtMapper.getProductLineByName(createProjectDemand.getProductLineName());
			if(apl == null){
			...
			if(anoleProjectExtMapper.countProjectByName(createProjectDemand.getProjectName()) == 0){
	...
	public List<Project> getProjects() {
		...
		List<AnoleProject> resultFromDB = anoleProjectMapper.selectByExample(new AnoleProjectExample());
	...
	public List<String> getProductLines() {
		...
		List<AnoleProductLine> resultFromDB = anoleProductLineMapper.selectByExample(new AnoleProductLineExample());
		if(resultFromDB!=null){
	...
	public List<String> getEnvs() {
		...
		if(result != null && !result.isEmpty())
			return result;
		result = new ArrayList<String>();
		List<AnoleEnvironment> resultFromDB = anoleEnvironmentMapper.selectByExample(new AnoleEnvironmentExample());
		if(resultFromDB != null){
