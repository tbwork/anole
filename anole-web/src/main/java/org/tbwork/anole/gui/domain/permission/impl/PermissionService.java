...
import org.anole.infrastructure.dao.extend.AnoleUserProjectMapExtMapper;
...
public class PermissionService implements IPermissionService {
	...
	@Autowired
	private AnoleUserProjectMapExtMapper anoleUserProjectMapExtMapper;
	...
	public AssignPermissionResult assignPermission(AssignPermissionDemand permissionDemand) {
		...
		try{
			...
			if(!permissionDemand.getRole().equals("owner")){
				...
				AnoleProject ap = anoleProjectExtMapper.getProjectByName(permissionDemand.getProjectName());
	...
	public int getUserRole(String project, String user, String env) {
		try{ 
			...
			if(permission != null)
				return permission; 
			AnoleProject ap = anoleProjectExtMapper.getProjectByName(project);
			if(ap!=null && ap.getOwner().equals(user))
				return 3;
			return anoleUserProjectMapExtMapper.getRoleByProjectKeyEnv(user, project, env);
		}
		catch(Exception e){
			return 0;
		} 
